...
export class DirectionsService {
    ...
    async getDirections(placeOriginId: string, placeDestinationId: string) {
        const requestParams: DirectionsRequest = {
            params: {
        ...
        const { data } = await this.googleMapsClient.directions(requestParams);
        return {
            ...data,
            request: {
                origin: {
                    place_id: requestParams.params.origin,
                    location: {
                ...
                destination: {
                    place_id: requestParams.params.destination,
                    location: {
                        lat: data.routes[0].legs[0].end_location.lat,
                        lng: data.routes[0].legs[0].end_location.lng
                    }
                },
                mode: requestParams.params.mode
            }
        }
    }
